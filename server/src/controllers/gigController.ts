...
export const getGigs = async (req: Request, res: Response) => {
    try {
        ...
        if (sort) {
            if (sort === 'price_asc') gigsQuery = gigsQuery.sort({ price: 1 });
            if (sort === 'price_desc') gigsQuery = gigsQuery.sort({ price: -1 });
            if (sort === 'newest') gigsQuery = gigsQuery.sort({ createdAt: -1 });
        }
...
export const updateGig = async (req: any, res: Response) => {
    try {
...
export const deleteGig = async (req: any, res: Response) => {
    try {
        const gig = await Gig.findById(req.params.id);
